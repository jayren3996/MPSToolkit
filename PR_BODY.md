...
**Kernel.** Basis-free truncation sizing the complement budget as `chi' = maxdim - 2 d^(2n)`, so the protected block fits inside the budget instead of being clipped. This reserves one direction more than the reference implementation (Jack Kemp's `dmt`, `DMT.h:719-720`, `subMaxDim -= sdimL + sdimR - 1`), deliberately: that bound holds only while the rank-one trace connector is active, and on a *traceless* operator -- a transport current -- the connector is disabled, `B = S` no longer annihilates the identity directions, and the reinstated rank saturates `maxdim`. Reserving the tighter figure makes the repair SVD clip a protected direction there (measured: the trace of a traceless operator drifts to 2e-3). The reference implementation has no such case to be correct for -- its connector divides by `D[1,1]` unguarded, which is ~0 exactly where this kernel switches it off. `conj` on the left protected block, matching the paper's `M = Q_L^T s Q_R` transpose pairing — silently correct for Hermitian operators, ~50% wrong otherwise.

**API.** `connector_buffer` removed (raises an `ArgumentError` naming its replacement); `preserve_diameter` (odd, default 3) replaces it. `maxdim` is now the **total** bond dimension inclusive of the protected block. Floors: 9 / 19 / 33 at `d = 2 / 3 / 4`.
