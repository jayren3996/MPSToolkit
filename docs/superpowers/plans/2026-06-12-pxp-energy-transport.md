...
- [x] **Step 1: Failing tests** — dense oracles in `test/test_pxp.jl`:
...
- [x] **Step 2: Run, verify failure** (`UndefVarError: pxp_term_hamiltonian`).

- [x] **Step 3: Implement `src/models/pxp.jl`:**
...
- [x] **Step 4: Wire include + exports, run `julia --project=. test/test_pxp.jl` → PASS.**
- [x] **Step 5: Commit** `feat(models): add PXP term and constraint-MPO helpers`.
...
- [x] **Step 3: Implement.** `vectorize.jl`:
...
- [x] **Step 5: Commit** `feat(operator-space): add Pauli vectorizers and PXP constraint objects`.
...
- [x] **Step 1: Failing tests.** Build a random dense Hermitian `ρ_d` on N=3 (`R + R†` over the
...
- [x] **Step 2: Run, verify failure.**
- [x] **Step 3: Implement.** Cap tensor `cap[α…] = tr(P_α O)` over the window (loop
...
- [x] **Step 4: Run tests → PASS.**
- [x] **Step 5: Commit** `feat(operator-space): trace and local-expectation sweeps for Pauli MPS`.
...
- [x] **Step 4: Run tests → PASS.**
- [x] **Step 5: Commit** `feat(operator-space): imaginary-time Gibbs/domain-wall preparation`.
...
- [x] **Step 1: Failing tests.** N=6 PXP ED oracle:
...
- [x] **Step 4: Run tests → PASS (tune atol to observed Trotter error with margin).**
- [x] **Step 5: Commit** `feat(operator-space): checkpointed constraint-projected DMT driver`.
...
- [x] **Step 1: Example script.** N=40, wall 20|21, `β=0.25`, `dt=0.1`, `maxdim=32`,
...
- [x] **Step 2: Run the example with quick parameters** (small N/maxdim override block at top,
e.g. N=16, 10 sweeps) to confirm it executes end to end.
- [x] **Step 3: Docs.** `dmt.md`: "Worked example: constrained energy transport in the PXP
...
- [x] **Step 4: Full verification.** `julia --project=. -e 'using Pkg; Pkg.test()'` → all green
(docstring coverage test included). Re-run `julia --project=. test/test_pxp.jl` standalone.
- [x] **Step 5: Commit** `feat(examples,docs): PXP energy-transport example and manual section`.
